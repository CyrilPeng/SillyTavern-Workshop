...
*   **💉 智能注入**：独家黑科技！自动识别资源类型，将世界书词条或者数据直接插入当前绑定的世界书和本地存储中，无需任何手动配置。
...
### 1. 启动插件
在 SillyTavern 左下角的扩展（魔法棒按钮）中，点击 **「创意工坊」** 图标即可打开面板。
...
### 3. 下载与注入
1. 在 **「下载」** 标签页浏览 MOD 资源。
2. 找到心仪的资源，点击卡片“详情”可以查看详细描述。
3. **注入**：直接应用到当前酒馆环境（推荐）。
   - *世界书*：自动追加到当前选中的 Worldbook。
   - *聊天数据*：自动导入到 LocalStorage (聊天配置) 和 IndexedDB (大型数据库) 中。
4. **下载**：保存 JSON 文件到本地设备。
